...
if __name__ == '__main__':
    ...
    year = 2018
    while year < 2019:
        path_mother = os.path.join('H:\\001zone\\00张家口承德地区\\zc_evf\\cal_zc_evf\\evf_bands',str(year)) # 输入路径，文件夹
        #path_out = os.path.join('H:\\001zone\\00张家口承德地区\\cal_zc_evf\\month_bands',str(year))  # 输出路径，文件夹
        path_out = os.path.join('H:\\001zone\\00张家口承德地区\\zc_evf\\cal_zc_evf\\evf_season',str(year)) # 输出路径，文件夹
        ...
        autum = B9 + B10 +B11
        print(year)
        outpath_1 = path_out + os.sep + str(year) + '_winter_evf.tiff'
        writeTiff(winter, im_width, im_height, im_bands, im_geotrans, im_proj, outpath_1)
        outpath_2 = path_out + os.sep + str(year) + '_spring_evf.tiff'
        outpath_3 = path_out + os.sep + str(year) + '_summer_evf.tiff'
        outpath_4 = path_out + os.sep + str(year) + '_sutum_evf.tiff'
        
        writeTiff(spring, im_width, im_height, im_bands, im_geotrans, im_proj, outpath_2)        
        writeTiff(summer, im_width, im_height, im_bands, im_geotrans, im_proj, outpath_3)
        writeTiff(autum, im_width, im_height, im_bands, im_geotrans, im_proj, outpath_4)        
